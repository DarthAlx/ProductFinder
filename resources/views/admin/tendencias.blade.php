			  	...
			  	@if($tendencias)
			  		@foreach($tendencias as $tendencia)
						...
						<tr style="cursor: pointer;">
							<td>{{$tendencia->nombre}}</td>
							<td align="right" style="text-align: right;">
								<a class="waves-effect waves-light btn  modal-trigger red" href="#delete{{$tendencia->id}}"><i class="fa fa-trash"></i></a>
							</td>
						</tr>
					@endforeach
				@else
...
<!-- Modal Structure -->
  <div id="nuevo" class="modal">
  	<form action="{{ url('/agregar-tendencia') }}" method="post" enctype="multipart/form-data">
    <div class="modal-content">
      <h4>Añadir nuevo</h4>
			{!! csrf_field() !!}
			<div class="input-field col m8">
	          <input id="nombre" name="nombre" type="text" class="validate" value="{{old('nombre')}}" required>
	          <label for="nombre">Nombre de tendencia</label>
	        </div>
	        <div class="col m4">
...
@foreach($tendencias as $tendencia)
<!-- Modal Structure -->
  <div id="delete{{$tendencia->id}}" class="modal">
    <div class="modal-content">
      <h4>Eliminar tendencia ({{$tendencia->nombre}})</h4>
      <div class="input-field col m8">
      <p>¿Está seguro que desea eliminar este tendencia?</p>
  	  </div>
	  <div class="col m4 text-right">

     
		<form action="{{ url('/eliminar-tendencia') }}" method="post" enctype="multipart/form-data">
			{{ method_field('DELETE') }}
			{!! csrf_field() !!}
			<input type="hidden" name="eliminar" value="{{$tendencia->id}}" style="display: inline-block;">
